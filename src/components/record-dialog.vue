...
export default {
    ...
    return (
      <el-dialog props={{ visible, title }} on={{ close: () => { this.$emit('update:visible'); this.$emit('close') } }}>
        <header-btn props={{ border: true, activeIndex: activeIndex }} hideEdit>
          ...
          <el-button type="danger" size="mini" on={{ click: this.del }}>删除</el-button>
  ...
  methods: {
    ...
    submitForm () {
      this.$refs.autoForm.submitForm().then(({ index, ...res }) => {
        this.$api[this.api[this.currentRow ? 'update' : 'add']](res).then(res => {
          this.$refs.searchTable.request(true)
