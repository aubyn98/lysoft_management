<template>
  <div class="page">
    <header-btn
      border
      @change="editChange"
      :hide-edit="!$permission([{ mc: '物料销售开单', xg: true }])"
    ...
    >
      <el-button
        type="info"
        size="mini"
        v-if="$permission([{ mc: '物料销售开单审核与作废', xg: true }])"
      ...
      <div class="billTitle-right" style="left: 880px">
        <div class="billTitle-right-table">
          <edit-table
            ...
            header-bg
            ref="editTable2"
            name="MaterialSalesBillingTitle"
    ...
    <content-table
      ...
      name="MaterialSalesBillingContentTable"
      v-show="tabIndex === '添加列表'"
    >
      <template #a>
        <edit-table
          ref="editTable"
          name="MaterialSalesBillingLeft"
      ...
      <template #b>
        <edit-table
          ref="editTable3"
          name="MaterialSalesBillingRight"
...
export default {
  ...
  data () {
    return {
      ...
      autocompleteApi: {
        khmc: 'getMaterialSalesBillingCustomer'
      }
    }
  },
  created () {
